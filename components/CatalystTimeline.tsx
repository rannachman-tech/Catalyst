    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
        ...
        <p className="mt-1 text-[12px] text-fg-subtle max-w-md">
          {mode === "plain"
            ? "Looks quiet. We will surface anything that drops onto the calendar within minutes."
            : "Calendar is empty within the 90d horizon. Curated and Finnhub feeds polled · 30m TTL."}
        </p>
      </div>
    );
  }
          ...
          <div className="space-y-2.5">
            {g.items.map((c) => (
              <CatalystCard
                key={c.id}
                c={c}
                daysOut={daysBetween(today, parseISO(c.date))}
                mode={mode}
              />
            ))}
          </div>
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        style={
          impactLevel === "hi"
            ? { boxShadow: `inset 3px 0 0 rgb(${cssVar})` }
            : impactLevel === "med"
            ? { boxShadow: `inset 2px 0 0 rgb(${cssVar})` }
            : { boxShadow: `inset 1px 0 0 rgb(${cssVar})` }
        }
        aria-expanded={open}
      >
      ...
      {open && (
        <div className="mt-2 ml-2 rounded-md border border-border bg-surface-2 px-3 py-3 text-[12px] text-fg-muted leading-relaxed fade-in">
          ...
          <p className={c.summary ? "mt-2" : ""}>
            {mode === "plain" ? CATEGORY_PLAIN[c.category] : proDescription(c)}
          </p>
                ...
                {c.meta.last4Reactions.map((mv, i) => (
                  <span
                    ...
                    style={{
                      background:
                        mv >= 0
                          ? "rgb(var(--phase-quiet) / 0.12)"
                          : "rgb(var(--phase-heavy) / 0.12)",
                      color:
                        mv >= 0
                          ? "rgb(var(--phase-quiet))"
                          : "rgb(var(--phase-heavy))",
          ...
          {c.meta?.impliedMove !== undefined && (
            <div className="mt-3 text-[12px] text-fg-muted">
              ...
              <span className="font-mono text-fg">
                ±{(c.meta.impliedMove * 100).toFixed(1)}%
              </span>{" "}
...
function ImpactPill({ impact }: { impact: number }) {
  ...
  const cls =
    tier === "hi"
      ? "bg-cat-earn/12 text-cat-earn"
      : tier === "med"
      ? "bg-cat-opt/12 text-cat-opt"
